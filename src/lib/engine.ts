...
import { CATALOG, BUNDLES, CITIES } from './data'
...
function parseBudget(text: string): number | null {
  ...
  m = t.match(/(\d{4,6})\s*(?:ට|ta|rupees|rs)/i)
  if (m) return parseInt(m[1], 10)
...
function search({ cat, occ, budget }: { cat: string | null; occ: string | null; budget: number | null }): Product[] {
...
export function respond(text: string, ctx: { cartCount: number; lastVimp?: string | null }): EngineResponse {
  ...
  if (/gift message|card message|what.*write|message.*card|sandeshaya|liyanna/i.test(text)) {
    return { lang, text: L(lang, {
      ...
      tl: 'Lassanama note ekak liyanna mama udaw karannam! ✍️ Baduwak cart ekata daala, checkout ekedi warm, witty nætnam formal message ekak hadamu.' }),
  ...
  if (cat || occ || budget || /gift|present|buy|find|show|need|want|තෑග|gift ekak|baduwak/i.test(text)) {
    ...
    const occLabel: Record<string,string> = { mother:'your amma', father:'your thaaththa', birthday:'the birthday', anniversary:'the anniversary', valentine:'your special someone', avurudu:'Avurudu', graduation:'the graduate' }
    const bridge = L(lang, {
      en: `Here are some lovely picks${occ ? ' for ' + occLabel[occ] : ''}${budget ? ' under Rs. ' + budget.toLocaleString('en-LK') : ''} 🎁 — tap Add to drop one in your cart:`,
    ...
    return { lang, text: bridge, card: { type: 'carousel', items },
